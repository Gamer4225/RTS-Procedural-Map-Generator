...
void FRTSGrid::Initialize(int32 InWidth, int32 InHeight, float InCellSize)
{
    ...
    // Pre-allocate exact size to avoid incremental TArray growth
    ...
    for (int32 Y = 0; Y < Height; ++Y)
    {
        ...
        {
            int32 Idx = Y * Width + X; // Inline ToIndex — avoid check() overhead during init
...
{
    // Clear without deallocating capacity — caller can reuse the TArray
    OutNeighbors.Reset();
    ...
    // 4-directional (clockwise: Left, Right, Up, Down)
    ...
    if (bDiagonal)
    {
        const bool bCanUp   = (Y > 0);
        const bool bCanDown = (Y < Height - 1);
        const bool bCanLeft = (X > 0);
        const bool bCanRight = (X < Width - 1);

        if (bCanUp && bCanLeft)    { OutNeighbors.Add(Index - Width - 1); }
        if (bCanUp && bCanRight)   { OutNeighbors.Add(Index - Width + 1); }
        if (bCanDown && bCanLeft)  { OutNeighbors.Add(Index + Width - 1); }
        if (bCanDown && bCanRight) { OutNeighbors.Add(Index + Width + 1); }
    }
}
...
int32 FRTSGrid::GetNeighborsFixed(int32 Index, bool bDiagonal, int32 OutFixedArray[8]) const
{
    ...
    if (bDiagonal)
    {
        ...
        if (bCanUp && bCanLeft)    { OutFixedArray[Count++] = Index - Width - 1; }
        if (bCanUp && bCanRight)   { OutFixedArray[Count++] = Index - Width + 1; }
        if (bCanDown && bCanLeft)  { OutFixedArray[Count++] = Index + Width - 1; }
        if (bCanDown && bCanRight) { OutFixedArray[Count++] = Index + Width + 1; }
    }
